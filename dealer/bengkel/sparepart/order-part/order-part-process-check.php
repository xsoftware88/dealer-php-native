...
//edit data
    $dataSelect = "SELECT * FROM part_order_tam_temp WHERE cabang = 'IDR' AND kode_order LIKE '%".$_POST['nomerOrder']."%'";
    ...
    foreach ($stmt->fetchAll() as $val) {
        ...
        $sql = "UPDATE part_order_tam_temp SET kode_order='".$tmp."' WHERE cabang = 'IDR' AND kode_order LIKE '%".$_POST['nomerOrder']."%'";
        // Prepare statement
...
//cetak data ambil dari database
    ...
    $dataSelect = "SELECT * FROM part_order_tam_temp WHERE cabang = 'IDR'";
    $stmt = $conn->prepare($dataSelect);
    $stmt->execute();
    foreach ($stmt->fetchAll() as $val) {
...
?>
    <script type="text/javascript">
        window.location = "http://192.168.0.2/dealer/bengkel/sparepart/order-part/order-part-cetak-txt.php";
    </script>
<?php
}
